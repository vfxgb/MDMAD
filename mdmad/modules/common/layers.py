...
class DistanceToBins(nn.Module):
    ...
    def __init__(self, dist_min=0.0, dist_max=20.0, num_bins=64, use_onehot=False):
        ...
        if use_onehot:
            ...
            offset = torch.linspace(
                dist_min, dist_max, self.num_bins - 1
            )  # 1 overflow flag
            self.coeff = (
    ...
    def forward(self, dist, dim, normalize=True):
        ...
        if self.use_onehot:
            diff = torch.abs(
                dist - self.offset.view(*offset_shape)
            )  # (N, *, num_bins, *)
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, num_funcs=6):
        ...
        self.register_buffer(
            "freq_bands", 2.0 ** torch.linspace(0.0, num_funcs - 1, num_funcs)
        )
...
class AngularEncoding(nn.Module):
    ...
    def __init__(self, num_funcs=3):
        ...
        self.register_buffer(
            "freq_bands",
            torch.FloatTensor(
                [i + 1 for i in range(num_funcs)]
                + [1.0 / (i + 1) for i in range(num_funcs)]
            ),
        )

...
def compute_global_R_from_standard_to_vec(vec):
    ...
    R = R + skew + torch.matmul(skew, skew) / (1.0 + c[:, None, None])
...
def grassmann_product(q_1: torch.Tensor, q_2: torch.Tensor):
    ...
    G[:, :, 0] = q_1[:, :, 0] * q_2[:, :, 0] - torch.sum(
        q_1[:, :, 1:] * q_2[:, :, 1:], dim=-1
    )
    G[:, :, 1:] = (
        q_1[..., 0].unsqueeze(-1) * q_2[..., 1:]
        + q_2[..., 0].unsqueeze(-1) * q_1[..., 1:]
        + torch.cross(q_1[..., 1:], q_2[..., 1:], dim=-1)
    )
    return G
...
def quaternion_diff(q_1_normalized: torch.Tensor, q_2_normalized: torch.Tensor):
    ...
    q_2_star = torch.zeros(
        (N, L, 4), dtype=q_2_normalized.dtype, device=q_2_normalized.device
    )

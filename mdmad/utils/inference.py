...
def get_residue_first_last(data):
    ...
    residue_first = (
        data["chain_id"][idx_first],
        data["resseq"][idx_first].item(),
        data["icode"][idx_first],
    )
    residue_last = (
        data["chain_id"][idx_last],
        data["resseq"][idx_last].item(),
        data["icode"][idx_last],
    )
    ...
    def __call__(self, data):
        ...
        if self.remove_sequence:
            data["aa"] = torch.where(
                generate_flag,
                torch.full_like(
                    data["aa"], fill_value=int(constants.AA.UNK)
                ),  # Is loop
                data["aa"],
            )

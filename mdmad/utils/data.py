...
class PaddingCollate(object):
    ...
    def _get_pad_mask(l, n):
        return torch.cat(
            [torch.ones([l], dtype=torch.bool), torch.zeros([n - l], dtype=torch.bool)],
            dim=0,
        )
    ...
    def __call__(self, data_list):
        ...
        for data in data_list:
            data_padded = {
                k: (
                    self._pad_last(v, max_length, value=self._get_pad_value(k))
            ...
            data_padded["mask"] = self._get_pad_mask(
                data[self.length_ref_key].size(0), max_length
            )

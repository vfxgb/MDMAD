...
from mdmad.tools.relax.base import RelaxTask
...
def _is_in_the_range(ch_rs_ic, flexible_residue_first, flexible_residue_last):
    if ch_rs_ic[0] != flexible_residue_first[0]:
        return False
    r_first, r_last = tuple(flexible_residue_first[1:]), tuple(
        flexible_residue_last[1:]
    )
    rs_ic = ch_rs_ic[1:]
...
class ForceFieldMinimizer(object):
    ...
    def _minimize(
        self, pdb_str, flexible_residue_first=None, flexible_residue_last=None
    ):
        ...
        if flexible_residue_first is not None and flexible_residue_last is not None:
            for i, a in enumerate(pdb.topology.atoms()):
                ch_rs_ic = (
                    a.residue.chain.id,
                    int(a.residue.id),
                    a.residue.insertionCode,
                )
                if (
                    not _is_in_the_range(
                        ch_rs_ic, flexible_residue_first, flexible_residue_last
                    )
                    and a.element.name != "hydrogen"
                ):
    ...
    def _add_energy_remarks(self, pdb_str, ret):
        ...
        pdb_lines.insert(
            1, "REMARK   1  FINAL ENERGY:   {:.3f} KCAL/MOL".format(ret["efinal"])
        )
        pdb_lines.insert(
            1, "REMARK   1  INITIAL ENERGY: {:.3f} KCAL/MOL".format(ret["einit"])
        )
        return "\n".join(pdb_lines)
    ...
    def __call__(
        ...
        pdb_min, ret = self._minimize(
            pdb_fixed, flexible_residue_first, flexible_residue_last
        )
...
def run_openmm(task: RelaxTask):
    ...
    try:
        ...
        logging.warning(f"{e.__class__.__name__}: {str(e)} ({task.current_path})")
        task.mark_failure()
    return task
